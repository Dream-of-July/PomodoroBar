...
struct LegacyPomodoroPanelView: View {
    ...
    private var rhythmEditor: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            VStack(spacing: 6) {
                Slider(value: rhythmLengthBinding, in: Double(LegacyPomodoroTimerStore.rhythmLengthRange.lowerBound)...Double(LegacyPomodoroTimerStore.rhythmLengthRange.upperBound), step: 1)
                    .labelsHidden()
                    .tint(.accentColor)
                    .controlSize(.regular)
                    .padding(.vertical, 3)
